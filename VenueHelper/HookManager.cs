...
// Captures /random and /dice results from the game's log module and forwards
// them to the giveaway tracker. Lifted from the proven Elementalist hook.
public unsafe class HookManager
{
    ...
    private void RandomPrintLogDetour(RaptureLogModule* module, int logMessageId, byte* playerName, byte sex, StdDeque<TextParameter>* parameter, byte flags, ushort homeWorldId)
    {
        ...
        try
        {
            ...
            Plugin.Deathroll.OnRoll(fullName, roll, outOf, isDice: false);
            Plugin.BarGames.OnRoll(fullName, roll, outOf, isDice: false);
        }
        catch (Exception ex)
        {
    ...
    private void DicePrintLogDetour(RaptureLogModule* module, ushort chatType, byte* playerName, void* unused, ushort worldId, ulong accountId, ulong contentId, ushort roll, ushort outOf, uint entityId, byte ident)
    {
        try
        {
            ...
            Plugin.Deathroll.OnRoll(fullName, roll, outOf, isDice: true);
            Plugin.BarGames.OnRoll(fullName, roll, outOf, isDice: true);
        }
        catch (Exception ex)
        {
